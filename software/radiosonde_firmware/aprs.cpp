...
#ifdef used_ds18b20
  String aprs::create_comment(int altitude, int aprs_packet_number, int temperature, int mcu_vin, int solar_vin, int gps_speed, int gps_course, int number_of_gps_satellites, String additional_comment)
  {
    char comment_buffer[80];

    // altitude*3.28084 - for conversion from meter to feet
    sprintf(comment_buffer, "/A=%06d/F%dN%dT%dE%dY%dV%dC%dS%d %s" , int((altitude*3.28084)), payload_flight_number, aprs_packet_number, temperature, mcu_vin, solar_vin, gps_speed, gps_course, number_of_gps_satellites, additional_comment.c_str());

    return String(comment_buffer);
  }

#elif defined used_bme680
  String aprs::create_comment(int altitude, int aprs_packet_number, int bme680_temperature, int bme680_humidity, long int bme680_pressure, int bme680_gas_resistance, int mcu_vin, int solar_vin, int gps_speed, int gps_course, int number_of_gps_satellites, String additional_comment)
  {
    char comment_buffer[80];

    // altitude*3.28084 - for conversion from meter to feet
    sprintf(comment_buffer, "/A=%06d/F%dN%dT%dH%dP%6ldG%dE%dY%dV%dC%dS%d %s" , int((altitude*3.28084)), payload_flight_number, aprs_packet_number, bme680_temperature, bme680_humidity, bme680_pressure, bme680_gas_resistance, mcu_vin, solar_vin, gps_speed, gps_course, number_of_gps_satellites, additional_comment.c_str());
